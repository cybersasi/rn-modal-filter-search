...
export default class ModalFilterPicker extends Component {
  ...
  _addKey(key) {
    var value = this.state.filter.concat(key)
  ...
  _delKey(){
    if(this.state.filter.length > 0){
      var value = this.state.filter.slice(0, -1)
      this.onFilterChange(value)
      this.props.onInputChange(value)
      this.setState({
        filter: value,
  ...
  renderList = () => {
    ...
    return (
      <View style={[listContainerStyle || styles.listContainer,styles.viewContainer]}>
        <View style={styles.leftView}>
          {filter}
          {this.renderOptionList()}
        </View>
        <View style={[styles.rightView ,{width:"40%",height:'100%',justifyContent:"flex-end"}]}>                       
            <Keyboard 
                keyboardType="decimal-pad"
                style={{width:"40%", width:'95%'}}
                onClear={this._handleClear.bind(this)}
                onDelete={this._delKey.bind(this)}
                onKeyPress={this._addKey.bind(this)}
            />
        </View>
      </View>
    )
  ...
  onFilterChange = (text) => {
    ...
    const filter = text.toString().toLowerCase()
    ...
    const filtered = (!filter.length)
      ...
      : options.filter(({ searchKey, label, key }) => (
        0 <= label.toLowerCase().indexOf(filter) ||
          (searchKey && 0 <= searchKey.toString().toLowerCase().indexOf(filter))
      ))
